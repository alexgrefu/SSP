    ...
    public partial class Content
    {
        ...
        public string ToSearchIndex()
        {
            ...
            foreach (var element in positionedElements.OrderBy(e => e.PositionInContent))
            {
                if (element is Image)
                {
                    string img = ((Image) element).ToSearchIndex();
                    Text = Text.Insert(element.PositionInContent == 0 ? 0 : (element.PositionInContent - (6 * x) + seed), img);
                    // scad 1, deorece parserul adauga o pozitie la content cand gaseste o imagine
                    seed += (img.Length - 1);
                }
                else if (element is Table)
                {
                    string table = ((Table) element).ToSearchIndex();
                    Text = Text.Insert(element.PositionInContent == 0 ? 0 : (element.PositionInContent - (6 * x) + seed), table);
                ...
                else if (element is Note)
                {
                    ...
                    string note = ((Note) element).ToSearchIndex();
                    Text = Text.Insert(element.PositionInContent == 0 ? 0 : (element.PositionInContent - (6 * x) + seed), note);
                    /*//seed += note.Length; */
                    seed = seed + 1;
                }
                else if (element is Definition)
                {
                    string definition = ((Definition) element).ToSearchIndex();
                    // magic number alert
                    Text = Text.Insert(element.PositionInContent == 0 ? 0 : (element.PositionInContent - (6 * x) + seed), definition);
